...
import (
	...
	cerrors "github.com/Microsoft/kunlun/common/errors"
...
func (i interpolator) Interpolate(node interface{}, varsLookup varsLookup) (interface{}, error) {
	...
	case string:
		for _, name := range i.extractVarNames(typedNode) {
			...
			if err != nil {
				return nil, errors.New(err.Error() + fmt.Sprintf("Finding variable '%s'", name))
			}
...
func (t varsTracker) Get(name string) (interface{}, bool, error) {
	...
	if err != nil {
		return nil, false, errors.New(err.Error() + fmt.Sprintf("Interpolating variable '%s' definition options", name))
	}
...
func (t *varsTracker) ExtractDefinitions(obj interface{}) error {
	...
	for _, def := range t.defs.Definitions {
		if len(def.Type) > 0 {
			...
			if err != nil {
				return errors.New(err.Error() + "Getting all variables from variable definitions sections")
			}
		}
	}
...
func (t varsTracker) FoundVars(obj interface{}) (interface{}, error) {
	...
	for _, def := range t.defs.Definitions {
		...
		if err != nil {
			return nil, errors.New(err.Error() + "Getting all variables from variable definitions sections")
		}
		if found {
...
func (t varsTracker) Error() error {
	...
	if len(errs) > 0 {
		return cerrors.NewMultiError(errs...)
	}
...
func (t varsTracker) MissingError() error {
	...
	return errors.New(t.multiErr(t.missing).Error() + "Expected to find variables")
}
...
func (t varsTracker) ExtraError() error {
	...
	return errors.New("Expected to use variables")
}
